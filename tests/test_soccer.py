...
class B365APISync:
    ...
    def format_soccer_match(self, match):
        ...
        if time == 'HT' or (minutes == 45 and timer_type == '0'):
            ...
            try:
                ...
                if minutes >= 90:
                    # If we have additional time in second half
                    if ta > 0:
                        time_display = f"90+{ta}"
                    elif ts > 0:
                        # If we have seconds in overtime, convert to minutes
                        overtime_mins = (ts + 30) // 60  # Round up to nearest minute
                    ...
                    # If we have additional time in first half
                    if ta > 0:
        ...
        if time == 'HT':
            ...
            try:
                ...
                if '+' in time:
                    ...
                    return int(base_time) + int(extra_time) * 0.1  # Add fraction for extra time
                return int(time)
            except ValueError:
                return 0
...
def show_live_soccer_events():
    ...
    matches.sort(key=lambda match: api.get_match_time_value(match))  
    ...
    for match in matches:
        ...
        # Print match info
        print(api.format_soccer_match(match))
        print("----------------------------------------")
...
class TestSoccerFormat(unittest.TestCase):
    ...
    def test_first_half(self):
        ...
        expected = ("⚽ Barcelona - Real Madrid\n"
                    "📊 Score:3 (2:1)\n"
                    "⏰ 35' First Half")
        self.assertEqual(B365APISync().format_soccer_match(match), expected)
    ...
    def test_break(self):
        """Test formatting for a match at break"""
        match = {
            'home': {'name': 'Manchester City'},
            'away': {'name': 'Arsenal'},
            'ss': '3-1',
